...
lemma ι_ξ_k1_α : ι (ξ k1) α = -(x[4] * x[6]) - (x[5] * x[7]) := by
  ...
  norm_cast
  ring_nf
  simp [mul_two, ←smul_add, ←smul_sub, ←smul_neg, ←two_smul ℂ, smul_smul]

abbrev ι_ξ_k1_α' : Λ0 := -(x[4] * x[6]) - (x[5] * x[7])
...
abbrev ι_ξ_k2_α' : Λ0 := x[4] * x[5] - x[6] * x[7]
...
abbrev ι_ξ_k3_α' : Λ0 := (2⁻¹ : ℂ) • (-(x[4] * x[4]) + x[5] * x[5] + x[6] * x[6] - x[7] * x[7])
...
abbrev ι_ξ_j1_α' : Λ0 := -(x[0] * x[2]) - x[1] * x[3]
...
abbrev ι_ξ_j2_α' : Λ0 := x[0] * x[1] - x[2] * x[3]
...
abbrev ι_ξ_j3_α' : Λ0 := (2⁻¹ : ℂ) • (-(x[0] * x[0]) + x[1] * x[1] + x[2] * x[2] - x[3] * x[3])
...
abbrev ι_ξ_p0_α' : Λ0 := -(x[0] * x[7]) + x[1] * x[6] - x[2] * x[5] + x[3] * x[4]
...
abbrev ι_ξ_p1_α' : Λ0 := -(x[0] * x[6]) - x[1] * x[7] - x[2] * x[4] - x[3] * x[5]
...
abbrev ι_ξ_p2_α' : Λ0 := x[0] * x[5] + x[1] * x[4] - x[2] * x[7] - x[3] * x[6]
...
abbrev ι_ξ_p3_α' : Λ0 := -(x[0] * x[4]) + x[1] * x[5] + x[2] * x[6] - x[3] * x[7]
